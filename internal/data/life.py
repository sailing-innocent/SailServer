...
class Project(ORMBase):
    ...
    ddl = Column(TIMESTAMP, server_default=func.current_timestamp())
...
class ProjectData:
    ...
    mtime: float = field(default_factory=lambda: datetime.now().timestamp())
    ddl: float = field(default_factory=lambda: datetime.now().timestamp())
