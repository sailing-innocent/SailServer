...
def get_service_account_impl(db, service_account_id: int):
    service_account = db.query(ServiceAccount).filter(ServiceAccount.id == service_account_id).first()
    return read_from_service_account(service_account)

def query_service_account_by_name_impl(db, name: str):
    service_account = db.query(ServiceAccount).filter(ServiceAccount.name == name).first()
    return read_from_service_account(service_account)
...
def get_service_accounts_impl(db, skip: int = 0, limit: int = -1):
    ...
    return [read_from_service_account(service_account) for service_account in service_accounts]

def update_service_account_impl(db, service_account_id: int, service_account_update: ServiceAccountCreate):
    service_account = db.query(ServiceAccount).filter(ServiceAccount.id == service_account_id).first()
